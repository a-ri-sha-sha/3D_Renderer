...
void World::moveCamera(size_t camera_id, const Vector3d& direction, double speed) {
    assert(camera_id < cameras_.size() && "moveCamera: camera_id should be less than count of cameras");
...
void World::rotateCamera(size_t camera_id, double yaw, double pitch) {
    assert(camera_id < cameras_.size() && "rotateCamera: camera_id should be less than count of cameras");
...
const World::Vector3d& World::getCameraPosition(size_t camera_id) const {
    assert(camera_id < cameras_.size());
    return cameras_[camera_id].getPosition();
}

World::Quaterniond World::getCameraRotation(size_t camera_id) const {
    assert(camera_id < cameras_.size());
