...
Picture Renderer::make(const World& world, size_t camera_id) {
    ...
    z_buffer_.resize(picture.getWidth() * picture.getHeight(),
...
Matrix3d Renderer::transformTriangleToWorldSpace(const Matrix3d& triangle, const Vector3d& position,
...
Matrix3d Renderer::transformTriangleToViewSpace(const Matrix3d& triangle, const Camera& camera) {
    ...
    for (int i = 0; i < 3; ++i) {
        result.row(i) -= camera.getPosition().transpose();
    }

    Matrix3d view_matrix = camera.getViewMatrix();

    for (int i = 0; i < 3; ++i) {
        result.row(i) = (view_matrix * result.row(i).transpose()).transpose();
    }
...
void Renderer::renderTriangle(Picture& picture, const Camera& camera, const Triangle& triangle,
    ...
    Matrix3d world_triangle =
        transformTriangleToWorldSpace(triangle.points, obj_position, obj_rotation);
    ...
    for (const auto& clipped_triangle : triangles) {
        ...
        for (int i = 0; i < 3; ++i) {
            int x =
                static_cast<int>((projected_triangle.row(i).x() + 1.0) * 0.5 * picture.getWidth());
...
void Renderer::renderWireframe(Picture& picture, const Camera& camera, const Triangle& triangle,
    ...
    drawLine(picture, screen_points[0].x, screen_points[0].y, screen_points[1].x,
             screen_points[1].y, wireframe_color);
    drawLine(picture, screen_points[1].x, screen_points[1].y, screen_points[2].x,
             screen_points[2].y, wireframe_color);
    drawLine(picture, screen_points[2].x, screen_points[2].y, screen_points[0].x,
             screen_points[0].y, wireframe_color);
}
...
bool Renderer::isPointInTriangle(const sf::Vector2i& p,
    ...
    int s1 = sign(p, triangle[0], triangle[1]);
    int s2 = sign(p, triangle[1], triangle[2]);
    int s3 = sign(p, triangle[2], triangle[0]);

    bool b1 = s1 < 0;
    bool b2 = s2 < 0;
    bool b3 = s3 < 0;

    bool result = ((b1 == b2) && (b2 == b3));
...
Vector3d Renderer::planeLineIntersection(const Vector4d& plane, const Vector3d& point1,
...
bool Renderer::clipTriangle(const Vector4d& plane, const Matrix3d& triangle,
